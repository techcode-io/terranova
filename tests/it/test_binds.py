...
class TestTerraformConstruction:
    def test_missing_terraform_binary_triggers_fatal_exit(
        ...
        with pytest.raises(Exit):
            Terraform(tmp_path, plugin_cache_dir)
...
class TestTerraformCreateEnvFiltering:
    def test_allowed_prefixes_and_exact_names_forwarded(
        ...
        terraform = Terraform(tmp_path, plugin_cache_dir)
        terraform.graph()
    ...
    def test_tf_var_prefix_injected_from_variables(
        ...
        terraform = Terraform(
            tmp_path, plugin_cache_dir, variables={"region": "eu-west-1"}
        )
        terraform.graph()
    ...
    def test_tf_log_debug_only_when_verbose(
        ...
        terraform = Terraform(tmp_path, plugin_cache_dir, verbose=True)
        terraform.graph()
...
class TestTerraformInit:
    def test_init_default_args(
        ...
        Terraform(tmp_path, plugin_cache_dir).init()
...
class TestTerraformValidate:
    def test_validate_success_returns_valid_result(
        ...
        result = Terraform(tmp_path, plugin_cache_dir).validate()
        assert result.valid
    ...
    def test_validate_appends_json_flag(
        ...
        Terraform(tmp_path, plugin_cache_dir).validate()
...
class TestTerraformPlan:
    def _plan(
        ...
        *,
        ...
        return Terraform(tmp_path, plugin_cache_dir).plan(
            input=input,
            no_color=no_color,
    ...
    def test_plan_default_parallelism_omitted(
        ...
        self._plan(tmp_path, plugin_cache_dir)
        assert not any(
    ...
    def test_plan_custom_parallelism_included(
        ...
        self._plan(tmp_path, plugin_cache_dir, parallelism=4)
        assert "-parallelism=4" in fake_terraform_bin.captured_argv

    def test_plan_parallelism_none_omitted(
        self, tmp_path: Path, fake_terraform_bin: FakeTerraform, plugin_cache_dir: Path
    ) -> None:
        self._plan(tmp_path, plugin_cache_dir, parallelism=None)
        assert not any(
            a.startswith("-parallelism=") for a in fake_terraform_bin.captured_argv
        )
    ...
    def test_plan_out_path_included(
        ...
        self._plan(tmp_path, plugin_cache_dir, out=out_path)
    ...
    def test_plan_returns_change_summary(
        ...
        summary = self._plan(tmp_path, plugin_cache_dir)
    ...
    def test_plan_counts_planned_change_events(
        ...
        create = '{"type":"planned_change","change":{"action":"create"}}'
        update = '{"type":"planned_change","change":{"action":"update"}}'
        delete = '{"type":"planned_change","change":{"action":"delete"}}'
        fake_terraform_bin.set_stdout(f"{create}\n{update}\n{delete}\n{create}")
        summary = self._plan(tmp_path, plugin_cache_dir)
        assert summary.to_add == 2
        assert summary.to_change == 1
...
class TestTerraformApply:
    def test_apply_default_no_flags_is_interactive(
        ...
        """
        summary = Terraform(tmp_path, plugin_cache_dir).apply()
        argv = fake_terraform_bin.captured_argv
        assert argv[0] == "apply"
    ...
    def test_apply_with_plan_arg(
        ...
        Terraform(tmp_path, plugin_cache_dir).apply(plan="path/to/plan")
        argv = fake_terraform_bin.captured_argv
    ...
    def test_apply_auto_approve_flag(
        ...
        Terraform(tmp_path, plugin_cache_dir).apply(auto_approve=True)
        argv = fake_terraform_bin.captured_argv
    ...
    def test_apply_returns_change_summary(
        ...
        summary = Terraform(tmp_path, plugin_cache_dir).apply(auto_approve=True)
...
class TestTerraformOutput:
    def test_output_captures_stdout_via_stringio(
        ...
        result = Terraform(tmp_path, plugin_cache_dir).output("some_name")
...
class TestTerraformOtherCommands:
    ...
    def test_destroy_invokes_destroy_arg(
        ...
        Terraform(tmp_path, plugin_cache_dir).destroy()
